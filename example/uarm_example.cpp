...
void test_move(Swift *swift) {
	...
	int ret = swift->set_position(200, -100, 80, 20000, false, false, 2, async_callback);
...
void range_motion_test(Swift *swift) {
  // Register callbacks
  swift->register_power_callback(power_callback);
  swift->register_report_position_callback(pos_report_callback);

  // Ensure power is on before continuing
  while (!swift->power_status) {
    int value = swift->get_power_status();
    cout << "get_power_status: " << value << endl;
    if (!swift->power_status) {
  ...
  for (int i = 0; i <= 10; i++) {
    for (int j = 0; j <= 10; j++) {
      for (int k = 0; k <= 10; k++) {
        ...
        ret = swift->set_position(x, y, z, 500, true, true, 5, NULL);
...
int main(int argc, char **argv) {
	...
	*/
	if (argc < 2) {
		// Print usage
		cout << "Usage: test_uarm [-e | {<serial port address>} ]" << endl;
		return -1;
	}
	std::string arg1(argv[1]);
	std::string port = "";
